(function () {
  ...
  function getScamIds() {
      try {
          const custom = window.vendetta.storage.customIds ?? [];
          return [
              ...DEFAULT_SCAM_IDS,
              ...custom
          ];
  ...
  function checkAndBan(message, guildId, channelId) {
      ...
      if (message.attachments?.length) {
          for (const att of message.attachments){
              ...
              const attId = att.id ?? att.url?.match(/\/attachments\/\d+\/(\d+)\//)?.[1] ?? "";
      ...
      try {
          const sendMessage = window.vendetta.metro.findByProps("sendMessage");
          sendMessage.sendMessage(channelId, {
              content: `?ban ${message.author.id}`
          });
      } catch (e) {
  ...
  function SettingsPage() {
      ...
      const { FormSection, FormRow, FormInput, FormDivider } = window.vendetta.ui.components.Forms;
      const storage = window.vendetta.storage;
      ...
      function removeId(id) {
          const updated = customIds.filter((x)=>x !== id);
          storage.customIds = updated;
          setCustomIds(updated);
      }
      return React.createElement(React.Fragment, null, // ── Default IDs (read-only) ──
      React.createElement(FormSection, {
          title: "Default Scam IDs (built-in)"
      }, ...DEFAULT_SCAM_IDS.map((id)=>React.createElement(FormRow, {
              key: id,
              label: id,
      ...
      customIds.length > 0 && React.createElement(FormSection, {
          title: "Custom Scam IDs"
      }, ...customIds.map((id)=>React.createElement(FormRow, {
              key: id,
              label: id,
  ...
  var index = {
      onLoad () {
          ...
          window.vendetta.ui.settings.registerSettings("AutoBanScammer", SettingsPage);
      },
      onUnload () {
          const { FluxDispatcher } = window.vendetta.metro.common;
